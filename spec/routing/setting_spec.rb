...
RSpec.describe 'routes for setting', type: :routing do
  it 'routes user setting' do
    expect(:get => '/gorums/setting').
        to route_to(:controller => 'setting', :action => 'show', :username => 'gorums')
  end

  it 'routes user setting with param' do
    expect(:get => '/gorums/setting/param').
        to route_to(:controller => 'setting', :action => 'show', :username => 'gorums', :val => 'param')
  end

  it 'bad request' do
    expect(:get => '/gorums/setting/param/param2').
        to route_to(:controller => 'errors', :action => 'routing', :a => 'gorums/setting/param/param2')
  end

  it 'routes user setting update profile' do
    expect(:patch => '/gorums/setting/profile').
        to route_to(:controller => 'setting', :action => 'profile', :username => 'gorums')
  end

  it 'routes user setting add email' do
    expect(:post => '/gorums/setting/account/add/email').
        to route_to(:controller => 'setting', :action => 'account_add_email', :username => 'gorums')
  end
